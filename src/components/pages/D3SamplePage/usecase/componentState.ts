...
import { D3SampleItem, FormProps } from '../types';
...
const toDataFromCSV = (text: string) => () => {
  ...
  const graphData = csvData
    .filter((d) => d.length > 1)
    .map((d) => ({ name: d[0].value, value: toValue(d[1].value) }));
  return graphData;
};

export const useComponentState = (props: D3SampleProps) => {
  ...
  React.useEffect(() => {
    if (d3Root.current) {
      updateElement(d3Root.current, d3sampleData);
    }
  }, [d3sampleData]);
